...
GLvoid capture_buffer() {
  char* buff;
  FILE* f;
  buff = (char*) calloc(w_width * w_height, 3);
  glReadPixels(0,0,w_width,w_height,GL_RGB,GL_BYTE,buff);

  char* fname;

  fname = (char*) calloc(16, sizeof(char));
  sprintf(fname, "ss/%05d.ppm", seq_num++);


  f = fopen(fname, "w");
  fprintf(f, "P6 %d %d 255\n", (int) w_width, (int) w_height);
  fwrite(buff, 3, w_width * w_height, f);
  fclose(f);
